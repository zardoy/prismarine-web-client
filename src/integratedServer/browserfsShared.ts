...
export const localFsState = {
  ...
  remoteBackend: false,
  saveLoaded: false,
  usingIndexFileUrl: null as null | string
}
...
fs.promises = new Proxy(Object.fromEntries(['readFile', 'writeFile', 'stat', 'mkdir', 'rmdir', 'unlink', 'rename', /* 'copyFile',  */'readdir'].map(key => [key, promisify(fs[key])])), {
  get (target, p: string, receiver) {
    ...
    return (...args) => {
      ...
      if (p === 'readFile') {
        localFsState.openReadOperations++
      } else if (p === 'writeFile') {
        localFsState.openWriteOperations++
      }
...
export const openWorldFromHttpDir = async (fileDescriptorUrls: string[]/*  | undefined */, baseUrlParam) => {
  ...
  localFsState.saveLoaded = false
  localFsState.isReadonly = true
  localFsState.syncFs = false
  localFsState.inMemorySave = false
  localFsState.remoteBackend = true
  localFsState.usingIndexFileUrl = indexFileUrl
